 # cut I
        ...
        con_27_e = @constraint(m, 
            sum(p[ω] * β[t,ω] for t in T for ω in Ω ) ≥
            sum(p[ω] * θʳ[t,ω] for t in T for ω in Ω ) -   
            sum(p[ω] * zʳ[t,ω][l] * δ[l] for t in T for ω in Ω for l in 1:length(zʳ[t,ω])  ) - 
            sum(p[ω] * θʳ[t,ω] for t in T for ω in Ω ) * sum(1- χ[h,j] for j in J for h in H¹[j] )
        )
        ...
        con_29_c = @constraint(m, 
            sum(p[ω] * β[t,ω] for t in T for ω in Ω ) ≥ 
            sum(p[ω] * θʳ[t,ω] for t in T for ω in Ω ) - 
            sum(p[ω] * ϵ[t,ω,j] for t in T for ω in Ω for j in Jʳ[t,ω] ) - 
            sum(p[ω] * θʳ[t,ω] for t in T for ω in Ω ) * sum(1 - χ[h,j] for j in J for h in H¹[j] )
        )
